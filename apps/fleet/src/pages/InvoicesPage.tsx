  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border p-6">
        <h2 className="text-sm font-medium text-gray-500">{t('invoices.billing.title')}</h2>
        <p className="text-xs text-gray-400 mb-4">{t('invoices.billing.hint')}</p>
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-gray-600">{t('invoices.number')}</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">{t('invoices.buyer')}</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">{t('invoices.load')}</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">{t('invoices.issued')}</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600 hidden md:table-cell">{t('invoices.due')}</th>
              <th className="text-right px-4 py-3 font-medium text-gray-600">{t('invoices.gross')}</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600">{t('invoices.status')}</th>
          ...
          <tbody className="divide-y">
            {invoices.map((inv) => (
              <tr key={inv.id}>
                ...
                <td className="px-4 py-3 text-gray-600">{inv.buyerName}</td>
            ...
            {invoices.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-500">{t('invoices.empty')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
