...
function RequestDetail({ req }: { req: HvacRequest }) {
  ...
  return (
    <div className="rounded-2xl bg-neu-raised p-5 shadow-neu">
      ...
      <p className="mt-1 text-sm text-ink-soft">{req.summary}</p>

      <div className="mt-4 rounded-xl bg-neu-inset p-4 shadow-neu-in">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-subtle">{t('hvac.extracted')}</p>
        <dl className="space-y-1.5 text-sm">
          <div className="flex justify-between gap-3"><dt className="text-subtle">{t('hvac.device')}</dt><dd className="font-medium text-ink">{req.device}</dd></div>
          <div className="flex justify-between gap-3"><dt className="text-subtle">{t('hvac.address')}</dt><dd className="text-right font-medium text-ink">{req.address}</dd></div>
          <div className="flex justify-between gap-3"><dt className="text-subtle">{t('hvac.urgency')}</dt><dd className={cn('font-semibold', URGENCY_CLS[req.urgency])}>{t(`hvac.urgency.${req.urgency}` as TranslationKey)}</dd></div>
        </dl>
      </div>
