  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="neu-card p-6 space-y-4">
        <p className="text-neu-muted text-sm">{t('import.desc')}</p>
        ...
        <div className="border-2 border-dashed border-neu-sunken rounded-xl p-8 text-center hover:border-brand-300 transition-colors">
          <Upload className="w-10 h-10 text-neu-muted/70 mx-auto mb-3" />
          <input
            type="file"
            accept=".csv,text/csv"
            ...
            className="text-sm"
          />
          {file && <p className="text-sm text-neu-muted mt-2">{file.name}</p>}
        </div>
        ...
        {result && (
          <div className="bg-neu-sunken/60 rounded-lg shadow-neu-inset-sm p-4 text-sm">
            ...
            {result.errors.length > 0 && (
              <div className="mt-2">
                ...
                <ul className="mt-1 space-y-1">
                  {result.errors.map((e) => (
                    <li key={e.row} className="text-neu-muted">{t('import.row')} {e.row}: {e.message}</li>
                  ))}
                </ul>
              </div>
            )}
            <Link to="/dashboard" className="text-brand-700 hover:underline mt-3 inline-block">
